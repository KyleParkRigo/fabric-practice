...
function deployChaincode() {
    ...
    CHAINCODE_PATH=${TEST_NETWORK_HOME}/bridge-chaincode/chaincode/mintertest/${CHAINCODE_NAME}
    ...
    for var in {0..0}; do
        ...
        echo ${PACKAGE_ID}
        set -x
        ${BIN_DIR}/peer lifecycle chaincode approveformyorg -o orderer0.example.com:7050 --ordererTLSHostnameOverride mdl1.orderer0.example.com --tls --cafile $ORDERER_CA $PEER_CONN_PARMS --channelID ${CHANNEL_NAME} --name ${CHAINCODE_NAME} --version ${CHAINCODE_VERSION} --package-id ${PACKAGE_ID} --sequence ${CHAINCODE_SEQUENCE} ${CHAINCODE_INIT_REQUIRED} ${CHAINCODE_END_POLICY} ${CHAINCODE_COLL_CONFIG} >&${LOG_PATH}
        res=$?
        { set +x; } 2>/dev/null
        cat ${LOG_PATH}
    ...
    for var in {0..0}; do
        ...
        set -x
        ${BIN_DIR}/peer lifecycle chaincode commit -o orderer0.example.com:7050 --ordererTLSHostnameOverride mdl1.orderer0.example.com --tls --cafile $ORDERER_CA $PEER_CONN_PARMS --channelID ${CHANNEL_NAME} --name ${CHAINCODE_NAME} --version ${CHAINCODE_VERSION} --sequence ${CHAINCODE_SEQUENCE} ${CHAINCODE_INIT_REQUIRED} ${CHAINCODE_END_POLICY} ${CHAINCODE_COLL_CONFIG} >&${LOG_PATH}
        res=$?
        { set +x; } 2>/dev/null
        cat ${LOG_PATH}
...
function main() {
    # ch1
    deployChaincode ch1 minter 1.0 1
    # deployChaincode ch1 chaincodematchinginfo-v2 1.0 1
    # deployChaincode ch1 validation-v2 1.0 1
    # deployChaincode ch1 ethswap-v2 1.0 1
    # deployChaincode ch1 mdlminter-v2 1.0 1
    # deployChaincode ch1 addressinfo-v2 1.0 1
    # # ch2
    # deployChaincode ch2 chaincodematchinginfo-v2 1.0 1
    # deployChaincode ch2 validation-v2 1.0 1
    # deployChaincode ch2 ethswap-v2 1.0 1
    # deployChaincode ch2 mdlminter-v2 1.0 1
    # deployChaincode ch2 addressinfo-v2 1.0 1
}
