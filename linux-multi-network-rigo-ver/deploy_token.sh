...
function deployChaincode() {
    ...
    CHAINCODE_PATH=${TEST_NETWORK_HOME}/rigo-bridge-chaincode/chaincode/${CHAINCODE_NAME}
    ...
    set -x
    ${BIN_DIR}/peer lifecycle chaincode package ${CHAINCODE_PACKAGE_PATH} --path ${CHAINCODE_PATH} --lang "golang" --label ${TOKEN_NAME}_${CHAINCODE_VERSION} >&${LOG_PATH}
    res=$?
    { set +x; } 2>/dev/null
    cat ${LOG_PATH}
    ...
    for var in {0..2}; do
        ...
        # --tls --cafile $ORDERER_CA --tlsRootCertFiles ${CORE_PEER_TLS_ROOTCERT_FILE}
        set -x
    ...
    for var in {0..0}; do
        ...
        { set +x; } 2>/dev/null
        cat ${LOG_PATH}
        PACKAGE_ID=$(sed -n "/${TOKEN_NAME}_${CHAINCODE_VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" ${LOG_PATH})
    ...
    for var in {0..2}; do
        ...
        while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
            ...
            set -x
            ${BIN_DIR}/peer lifecycle chaincode checkcommitreadiness --channelID ${CHANNEL_NAME} --name ${TOKEN_NAME} --version ${CHAINCODE_VERSION} --sequence ${CHAINCODE_SEQUENCE} ${CHAINCODE_INIT_REQUIRED} ${CHAINCODE_END_POLICY} ${CHAINCODE_COLL_CONFIG} --output json >&${LOG_PATH}
            res=$?
            { set +x; } 2>/dev/null
            let rc=0
            for var in "$@"; do
    ...
    for var in {0..0}; do
        ...
        while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
            ...
            set -x
            ${BIN_DIR}/peer lifecycle chaincode querycommitted --channelID ${CHANNEL_NAME} --name ${TOKEN_NAME} >&${LOG_PATH}
            res=$?
            { set +x; } 2>/dev/null
...
function main() {
    # ch1
    deployChaincode ch1 0x1111111111111111111111111111111111111111 1.0 1
    deployChaincode ch1 mdl1-ch1-mdm 1.0 1
    deployChaincode ch1 mdl1-ch1-rigo 1.0 1
    # ch2
    deployChaincode ch2 mdl1-ch2-mdm 1.0 1
    deployChaincode ch2 mdl1-ch2-rigo 1.0 1
}
